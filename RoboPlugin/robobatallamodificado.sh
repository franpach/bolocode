...
actualiza_color_codigo() {
	...
	then
		echo "No se puede cargar el fichero de colores. Se aplicarán los colores por defecto"
		...
		if [ "$enc" == "no" ]
		then
			echo "No se ha encontrado una configuración de colores para este robot. Aplicando por defecto..."
			newColor="setColors(Color.white,Color.white,Color.black);"
			sed -i 's/setColors.*/'$newColor'/g' ./robots/$1.java			
...
modificar_fichero_colores(){
	...
	then
		...
		echo "no" > c.txt
		cat "robots/colores.csv" | while IFS=, read -r robot 
		do
			if [ "$robot" = $robotName ]
			then
				echo "si" > c.txt
		...
		if [ "$si" = "si" ]
		then
			sed -i "s/$robotName,.*/$robotName,$colorBody,$colorGun,$colorRadar/g" robots/colores.csv
		else
			echo $robotName,$colorBody,$colorGun,$colorRadar >> robots/colores.csv # ponemos al final el nuevo color
		fi
		rm c.txt
	fi
}
